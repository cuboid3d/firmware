 ...
 * M280 - Set servo position absolute: "M280 P<index> S<angle|碌s>". (Requires servos)
...
static void clean_up_after_endstop_or_probe_move() {
 ...
 // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
  ...
  //Enable this if your SCARA uses 180掳 of total area
...
void loop() {
#if ENABLED(AUTOTEST)
    /* Auto test for manufacture */
    uint8_t i;
    uint8_t z_min_limited = 0;
    uint8_t z_max_limited = 0;

    enable_z();

    //Check Z_MIN&Z_MAX status, if Z_MIN or Z_MAX limited, 
    //Fan_0 and Fan_1 do not twinkle
    //z_motor do not rotate
    z_min_limited = endstops.min_z_is_limited();
    z_max_limited = endstops.max_z_is_limited();
    
    if (!z_min_limited) {
        // Open Fan_0 and Fan_1
    ...
    if (!z_max_limited) {
        // Motor rotation
        stepper.stepper_test();     
    } else {
        delay(1000);
    }

    // Close Fan_0 and Fan_1
    analogWrite(FAN_PIN, 0);
    analogWrite(FAN1_PIN, 0);
